  ...
  return (
    <li className="relative, mb-6 sm:mb-0 sm:pl-10">
        <div className="flex items-center">
            <div className={`z-10 flex items-center justify-center w-6 h-6 ${step?.status === 'completed' ? 'bg-green-900 text-green-100' : step?.status === 'pending' ? 'bg-red-700 text-red-100' : step?.status === 'processing' ? 'bg-blue-600 text-blue-100' : 'bg-indigo-900 text-indigo-100'} ${iconTextColor} rounded-full ring-0 ring-white shrink-0`}>
        ...
        <div className="mt-3 sm:pe-8">
            <h3 className = {`font-medium text-sm ${labelTextColor}`}>{step.label}</h3>
            <time className = "block mb-2 text-sm font-normal leading-nonetext-gray-400">{order.updatedAt ? new Date(order.updatedAt).toLocaleString(): 'Time'}</time>
